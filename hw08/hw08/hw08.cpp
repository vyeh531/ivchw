...
namespace hw08 {
    ...
    void print_2darray_pointer(double* twoDD, int row, int col)         // The place I made some corrections
 ...
 /*       for (int i = 0; i < row; i++)
        {
            //double* p = &twoDD[i * col];              // The place I made some corrections
            for (int j = 0; j < col; j++)
            {
                // our 2d array is layed out linearly in memory as contiguous rows, one after another, there are #row rows
                // each row has #col columns
...
*/
        ...
        // print array using pointer arithmetic
    {
        for (int i = 0; i < row; i++)
        {
            //int* p = twoDD[i];                      // The place I made some corrections
            for (int j = 0; j < col; j++)
            {
                ...
                cout << *(*(twoDD + i) + j) << " ";         // The place I made some corrections
            }
            cout << endl;
        }
        cout << endl;
...
int main()
{
    ...
    // double *q = &x; is not allowed because of type safety
    ...
    *p = 5;
    ...
    //double& s = x; is not allowed because of type safety
    ...
    //int& r3; is not allowed because references must be defined and declared at the same time
    ...
    *pd = 3;
    ...
    // because this will be out of range of the array
    //double d = *pd;// [2.4] // ... error explain
    // there's no number to d
    //*pd = 12.34;   // [2.5] // ... error explain
    // becaude pd cannot hold float type, it can only hold double value type
    ...
    // It is out of range of the array 5 + 7 > 10
    //int dif3 = pd2 - pd3;// [2.8] // ... error explain
    // pd2 and pd3 exist on different parts of the heap, and they are not contiguous
    ...
    // Q#4 - new, delete operator examples
    {
        ...
        //pi = pd;  // [4.5] pi points to pd
                    // ... error explain
        // pi = pointer of int type cannot be assigned pd (pointer of double type)
        //pd = pi;  // [4.6] pd points to pi
                    // ... error explain
        // Because of type safety, it prevents pointers of different types from assignment
...
/*
Q#1: What is a pointer. Describe what a reference is. What¡¦s the difference between a pointer and a reference?
A#1: Pointer is a type of variable that hold memory address of another variable, and it needs to be dereferenced 
      with * operator ro access the memory location. Reference is an alias variable, and it can be viewed as another 
      name of an existing variable, and it is also implemented by storing the address of an object. Hence, a 
      reference is similar to a const pointer. A pointer can be initialized to any value anytime after it is declared.
      A reference must be initialized when it is declared. A pointer can be changed to point to any variable of 
      the same type. Once a reference is initialized to a variable, it cannot be changed to refer to a variable object.

Q#3: How is the twoDDoubles 2d array laid out in memory? Why is it necessary to cast twoDDoubles to a double* in the 
       print_2darray_pointer function call?
A#3: A static two-dimensional array looks like an array of arrays - it's just laid out contiguously in memory. Although
        passing array by reference is safer than passing by pointer, the way of using pointers is more flexible, and so 
        we could save many code and memory since the compiler does not need to make a new data in the memory.

Q#5: How is the p_p_tictactoe 2d dynamic array laid out in memory (i.e. why is p_p_tictactoe an int**)? Why do we need 
        to pass a pointer to a pointer of type int in the print_2darray_dynamic_pointer function call?
A#5: Dynamic arrays are allocated on the heap at run time. The heap space can be assigned to global or 
        local pointer variables that store the address of the allocated heap space. We need to pass a pointer to a 
        pointer of type int in the print_2darray_dynamic_pointer function call so that we can change the value of the 
        pointer passed to a function as the function argument.
