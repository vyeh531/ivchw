...
namespace hw16 {
    ...
    public:
        ...
        bool operator() (int x) const { return x < v; } //functionally - uniary predicate) compare
    };
        ...
        // copy elements from [first,last) to result that sastisfy predicate
    {
        ...
        while (first != last) {
            if (pred(*first)) *result++ = *first;
                //Include an if statement with a conditional based upon whether the predicate
                // is satisfied to decide if a copy is made(i.e. * result++ = *first only if predicate is true).
    ...
    class Range_low_to_high {
        //meaning the range must be determined by using operator <
        Val low;
        Val high;
    public:
        Range_low_to_high(Val low1, Val high1) :low{ low1 }, high{ high1 }{}// store value
...
int main()
{
    ...
    cout << "*    CS1C T,Th 12:30pm - 2:50pm      * " << endl;
    cout << "************************************** " << endl;
    cout << endl;
    ...
    if (p!=v.end())
        cout << "find_if alg: found less_than_v (-100) value of in vector v: " << *p << endl << endl;
    else
        cout << "find_if alg: can't find less_than_v (-100) value in vector v" << endl << endl;
...
// Q#1
//      EqualityComparible must support == operator and the result should have 
//      standard semantics.To satisfy this requirement, types that do not have 
//      built - in comparison operators have to provide a user - defined operator==.
//      InputIterators only support once a LegacyInputIterator i has been 
...
//      It means that there are the type requirements which Input_iterator<In>, 
//      Output_iterator<Out>, Predicate<Pred> must need to make the function work
//  What operations must these types have such that copy_if will work ?
//      InputIt must meet the requirements of LegacyInputIterator. (once a 
//      LegacyInputIterator i has been incremented, all copies of its previous 
//      value may be invalidated.)
//      OutputIt must meet the requirements of LegacyOutputIterator.(X is a class 
//      type or a pointer type)
//      Predicate must meet the requirements of Predicate.(The Predicate requirements 
//      describe a callable that returns a value testable as a bool.
// 
